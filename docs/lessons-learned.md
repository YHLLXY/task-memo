...
> 2026-07-19 | 纯 HTML/CSS/JS PWA 项目 | 18 项功能 | 23 条经验
...
□ SW: 每次部署必须升级缓存版本号，否则手机端代码不更新
□ 布局: 固定元素过多会挤掉输入区 → sticky 放滚动区内部
□ PWA: 版本号 Toast（localStorage 比对）让用户知道更新已生效
□ 排序: 多级排序中手动调序字段必须是一级键，否则被上级覆盖
□ 排序: 手动调序后重分配所有 order 值为 0,1,2…
□ 排序: 新任务 order = minOrder-1（顶部）而非 Date.now()（底部）
```

---

## 十四、SW 缓存版本号 = 手机端唯一的更新通道

### 问题

手机端改完代码部署后，用户打开还是旧版。HTML 网络优先能拿到最新 HTML，但 JS/CSS 走缓存优先——旧缓存不清理，新代码永不生效。

### 教训

**#16：每次修改 JS/CSS 后，必须升级 sw.js 中的 CACHE_NAME 版本号。** 这是手机端看到新代码的唯一方式。

```javascript
// sw.js — 每次部署改这个
var CACHE_NAME = 'memo-v9';  // v8 → v9
```

同时 activate 事件中清理旧缓存，确保手机存储不被无限撑大。

### 教训

**#17：升级 SW 版本号后，配合版本 Toast 让用户感知更新。** 用户不知道后台发生了什么——看到"已更新到 v2.3.1"才知道新功能生效了。

实现：app.js 维护 VERSION 常量 → localStorage 比对 → 不同则弹出 3 秒 Toast。

---

## 十五、flex 布局过多固定元素会挤掉核心交互区

### 问题

在 flex 三区布局（header + scroll-area + input-area）中加入 date-nav（`flex-shrink: 0`）后，手机端输入区被挤出屏幕底部消失。

### 教训

**#18：flex 容器中 `flex-shrink: 0` 的元素越多，`flex: 1` 的滚动区越小。** 在 375×667 的手机屏幕上，每多一个固定行就吃掉约 40px。正确做法：

- 非核心控件放进 scroll-area 内部，用 `position: sticky; top: 0` 吸附顶部
- 减少 stats/header 的 padding 挤出更多内容空间
- 每个固定元素都要问：它真的需要一直可见吗？
...
用户在历史某天还能点 ○ 完成任务、点文字编辑、添加子任务。但过去日期的任务修改会造成数据混乱（比如完成了一个 3 天前的任务，它应该算哪天完成的？）。

### 教训

**#19：历史视图必须禁用所有修改入口。** 包括：完成按钮、内联编辑、排序箭头、子任务添加、子任务复选框。判断标准：`isTodayView()` 为 false 时全部只读。

```javascript
// render.js — renderTaskCard(t, index, total, isToday)
if (isToday) {
  // 完整的交互按钮
} else {
  // 只读展示，无可点击元素
...
**#20：高频操作应减少步骤。** 优先级修改如果做成"点击任务 → 进入详情 → 改优先级 → 保存"，步骤太多。直接放在长按菜单里，只显示与当前不同的两个选项，一次点击完成。2 步变 1 步。
...
`sortTasks` 用了 4 级排序：钉选 → 逾期 → 优先级 → order。`order` 是最低级 tiebreaker。`moveTask` 只交换两个任务的 `order` 值——但如果它们在逾期/优先级上不同，排序后自动规则会把它们拉回原位：

```
[🔴紧急 order:100] [🟡中等 order:200]     ← 两个任务优先级不同
点击🔴的 ▼ → 交换 order 值
[🔴紧急 order:200] [🟡中等 order:100]     ← order 换了
重新排序（优先级优先！）
[🔴紧急 order:200] [🟡中等 order:100]     ← 原地不动！order 被优先级覆盖
```

### 教训

**#21：多级排序中，手动调序的字段必须是一级键。** 把它降为 tiebreaker = 任何上级键有差异时手动调序都无效。

修复方案：砍掉逾期和优先级的自动排序，简化为 `钉选 → order`。逾期和优先级用**视觉元素**传达（红色文字、色条），不需要靠排序位置重复表达。

### 教训

**#22：手动调序后必须重分配所有 order 值。** 交换两个任务在数组中的位置 → `for` 循环从 0 开始重新赋 order = 0, 1, 2…。这样下次 `sortTasks` 时顺序一定正确，不会有"order 值乱了"的问题。
...
**#23：新任务 order = minOrder - 1（插入顶部），而非 Date.now()（插入底部）。** 用户新建任务时期望它出现在列表顶部，不是底部。