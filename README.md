...
| 📆 | 历史回溯 | ← → 箭头切换日期，查看近 30 天任意一天的任务（非今日只读） |
| 🧹 | 30 天清理 | 每次打开自动删除 30 天前的已完成任务 |
| 🛡 | 数据保护 | 页面退出/后台时自动防御性保存，防止数据丢失 |
...
| 日期 | 版本 | 内容 |
|------|------|------|
| 2026-07-19 | v2.3 | **历史回溯 + 数据保护 + 优先级修改** — 日期导航 ← → 查看近 30 天任务；防御性保存（visibilitychange/pagehide/beforeunload）防数据丢失；长按菜单修改优先级；saveRaw 错误日志 + 自动清理重试 |
